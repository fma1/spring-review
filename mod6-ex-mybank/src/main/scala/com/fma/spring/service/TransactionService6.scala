...
import com.fma.spring.service.TransactionService6.ROW_MAPPER
...
class TransactionService6(jdbcTemplate: JdbcTemplate) {
  ...
  def create(amount: BigDecimal, reference: String, receivingUser: String): Transaction6 = {
    ...
    val transaction = Transaction6(amount, reference, bankSlogan, receivingUser)
    transaction.setId(uuid)
    transaction
  }
}

object TransactionService6 {
  final val ROW_MAPPER: RowMapper[Transaction6] = (resultSet: ResultSet, _: Int) => {
    val transaction = new Transaction6()
